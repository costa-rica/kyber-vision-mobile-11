...
export default function ScriptingLive02({ navigation }) {
  ...
  const addActionToActionsArray = () => {
    ...
    setActionsArray([...actionsArray, newActionObj]);
  };

  const handleChangeType = (newType) => {
    if (actionsArray.length > 0) {
      let lastAction = actionsArray[actionsArray.length - 1];
      lastAction = { ...lastAction, type: newType };
      setActionsArray([...actionsArray.slice(0, -1), lastAction]);
    }
